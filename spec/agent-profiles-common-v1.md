...
7. **Seal time, as defense in depth:** the `sigTst` genTime of the bound
   ...
   `eventTime` later than its own seal time plus the TSA's stated accuracy
   and a five-second skew allowance → `eventTimesPlausible`. The allowance
   exists because producer hosts and TSAs disagree by seconds (vector 10
   was sealed by a TSA two seconds behind the producer's clock); minutes
   are not absorbed. `eventTime` stays the producer's claim; this only
